...
import type { TaskWithQuadrant } from '@/lib/eisenhower';
import { TagBadge } from '@/components/tags/TagBadge';

export interface EisenhowerTaskCardProps {
  task: TaskWithQuadrant;
  disabled?: boolean;
  onClick?: (task: TaskWithQuadrant) => void;
}
...
/**
 * Compact task card for Eisenhower Matrix.
 *
 * Displays task title, priority badge, tags, and due date.
 * Draggable when not disabled.
 */
export const EisenhowerTaskCard = memo(function EisenhowerTaskCard({
  task,
  disabled = false,
  onClick,
  ...
  const { setNodeRef, attributes, listeners, transform, isDragging } = useDraggable({
    id: task.id,
    disabled: disabled || task.status === TaskStatus.CANCELLED || task.status === TaskStatus.DONE,
    data: {
      type: 'task',
      taskId: task.id,
  ...
  // Quadrant colors
  const quadrantColors: Record<TaskWithQuadrant['quadrant'], { bg: string; border: string }> = {
    'do-first': { bg: 'bg-error/5', border: 'border-error/30' },
    schedule: { bg: 'bg-warning/5', border: 'border-warning/30' },
    delegate: { bg: 'bg-info/5', border: 'border-info/30' },
    eliminate: { bg: 'bg-text-tertiary/5', border: 'border-text-tertiary/20' },
  };
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isCancelled && 'opacity-50 bg-background-secondary',
